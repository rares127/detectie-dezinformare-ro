...
Scop: Aplicăm IG (Sundararajan et al. 2017) pe aceleași 4 grupuri × 25 articole
ca în L1a, pentru comparație head-to-head cu LIME.

Avantaje IG vs LIME:
  - Determinist (zero variabilitate între rulări cu același seed)
  - Principled axiomatic (Completeness, Sensitivity, Implementation Invariance)
  - Funcționează pe gradient → bypass natural al saturării softmax
  - Mai rapid: ~50 forward+backward passes vs ~1000 LIME forward passes

Dezavantaje:
  - Atribuțiile depind de baseline (alegere de design)
  - Atribuțiile sunt per-token BPE → trebuie agregate la nivel de cuvânt

Design:
  - Baseline: PAD tokens (recomandare literatură pentru transformere)
  - Layer: model.roberta.embeddings.word_embeddings
  - Steps: 50 (Sundararajan et al. recomandă 20-300; 50 e standard)
  - Aceleași 100 articole ca L1a (selectare prin același seed)

Metrici per articol:
  - top_features_ig: top-15 cuvinte cu atribuții pozitive (ca LIME)
  - faithfulness_deletion_auc: aceeași definiție ca L1a (drop predicție când ștergem top-k)
  - attribution_completeness: verificare axiomă (suma atribuțiilor ≈ f(input) − f(baseline))
...
Output:
  ...
  - ig_html_l3/ — vizualizări HTML cu atribuții colorate (5 per grup)
...
# Coloana cu textul input (același ca în L1a)
COLOANA_TEXT = "text_curat"

# Configurație IG
IG_STEPS = 200  # Crescut de la 50 — necesar pentru convergență pe XLM-R fine-tuned
...
def construieste_predict_proba(model, tokenizer, device, max_length=256, batch_size=16):
    ...
    Identică cu cea din L1a — pentru comparație directă.
    """
...
def calculeaza_ig_pe_articol(text, label_pred, model, tokenizer, device,
    ...
    """
    Calculează atribuții IG pentru un articol și returnează:
      - cuvinte_atributii: list de (cuvânt, atribuție) ordonat cum apar în text
      - top_features: top-K cuvinte după |atribuție| (pentru comparație cu LIME)
    ...
    Mecanică:
      1. Tokenizăm input + construim baseline (zero embeddings sau PAD ids)
      2. LayerIntegratedGradients pe embedding layer cu n_steps interpolări
      3. internal_batch_size pentru a evita OOM la n_steps mare
      4. Atribuții per token (vector 768d) → sumă cu semn
      5. Agregare token → cuvânt prin offset_mapping

    Args:
        baseline_mode: "zero" (embeddings zero, recomandat) sau "pad" (token PAD)
        internal_batch_size: cât de multe pași IG procesăm odată (mic = lent, mult = OOM risk)
    """
    ...
    # Construim baseline-ul în funcție de mod
    if baseline_mode == "zero":
        # Baseline = zero embeddings (referință neutră autentică)
        # LayerIntegratedGradients gestionează asta automat dacă pasăm input_ids ca baseline
        # și atașăm hook pe layer-ul de embedding cu zerorizare
        # ABORDARE STANDARD CAPTUM: pasăm baseline_ids = pad_id pe toată lungimea
        # DAR layer.attribute() face interpolarea PE EMBEDDINGS, nu pe IDs
        # → dacă vrem zero embeddings ca baseline, folosim un input cu PAD și
        #   modelul va produce embeddings PAD (NU zero) → asta nu e zero embeddings
        # SOLUȚIA CORECTĂ pentru zero baseline pe LayerIntegratedGradients:
        # construim un input fictiv cu PAD și separat trecem baseline=tensor de zerouri
        # de aceeași dimensiune ca embeddings layer output

        # Implementare practică: folosim PAD ids ca baseline_ids și DEPENDEM
        # de faptul că PAD embedding e foarte mic în XLM-R (verificăm empiric)
        # PENTRU COMPARABILITATE TOTALĂ cu literatura, păstrăm aceeași secvență
        baseline_ids = torch.full_like(input_ids, pad_id)
        # Păstrăm CLS și SEP la pozițiile lor (sunt întotdeauna la început/sfârșit)
        baseline_ids[0, 0] = cls_id
        # SEP e la prima poziție unde apare în input_ids (înainte de padding)
    ...
    # Definim funcția forward pentru Captum (returnează doar logitul clasei prezise)
    ...
    # Setăm IG pe embedding layer
    embedding_layer = model.roberta.embeddings.word_embeddings
    lig = LayerIntegratedGradients(forward_func, embedding_layer)

    # Calculăm atribuțiile cu internal_batch_size pentru a evita OOM la n_steps mare
    ...
    # Reducem dimensiunea hidden cu sumă (semnul contează: pozitiv = împinge spre label_pred)
    # Captum recomandă: attributions.sum(dim=-1) sau norm(dim=-1) cu semn
    # Folosim suma (păstrează semnul; poți compara cu logitul total)
    attr_per_token = attributions.sum(dim=-1).squeeze(0).cpu().numpy()  # (seq_len,)

    # Probabilități pe input real și baseline (pentru sanity check completeness)
    with torch.no_grad():
        logit_input = model(input_ids=input_ids, attention_mask=attention_mask).logits[0, label_pred].item()
        # Pentru baseline folosim atenția input-ului real (consistent cu modul în care a fost atribuit)
        logit_baseline = model(input_ids=baseline_ids, attention_mask=attention_mask).logits[0, label_pred].item()

    # Sanity check: suma atribuțiilor ≈ logit_input − logit_baseline (axioma Completeness)
    ...
    # Probabilitățile (pentru raportare)
    with torch.no_grad():
...
def agrega_tokens_la_cuvinte(text, offsets, attr_per_token, input_ids, tokenizer):
    ...
    XLM-R folosește SentencePiece BPE — un cuvânt poate fi împărțit în mai mulți tokeni.
    Sumăm atribuțiile tokenilor aceluiași cuvânt (delimitat de spațiu sau punctuație).

    Args:
        text: textul original
        offsets: array de shape (seq_len, 2) cu (start_char, end_char) per token
        attr_per_token: array de shape (seq_len,) cu atribuții per token
        input_ids: array de shape (seq_len,) cu IDs (pentru a sări tokeni speciali)
        tokenizer: tokenizer-ul HuggingFace

    Returns:
        list de (cuvânt, atribuție_agregată) în ordinea apariției în text
    """
    # Identificăm tokenii speciali pe care îi sărim
    special_ids = set()
...
def calculeaza_faithfulness_deletion_ig(text, top_features, predict_proba, label_pred,
    ...
    """
    Identică cu funcția din L1a — folosim cuvintele identificate de IG (în loc de LIME).
    Permite comparație head-to-head în același tabel.
    """
...
def selecteaza_grupuri(baseline_preds, baseline_test, loso_preds, loso_test,
                        n_per_group, seed):
    """
    IDENTIC cu L1a — selectează aceleași 4 grupuri × n_per_group articole.
    Folosim același seed → exact aceleași articole ca în L1a → comparație directă.
    """
...
def salveaza_html_atributii(text, cuvinte_atributii, label_pred, label_true,
    ...
    """
    Salvează un HTML cu atribuțiile IG colorate (verde = pozitiv, roșu = negativ).
    Vizualizare similară cu LIME explainer dar pentru atribuții IG.
    """
    # Normalizăm atribuțiile pentru intensitate culoare
    ...
    for cuv, attr in cuvinte_atributii:
        ...
        # Verde pentru pozitiv (împinge spre label_pred), roșu pentru negativ
        if attr > 0:
...
def proceseaza_grup(nume_grup, df_grup, model, tokenizer, device, html_dir,
    ...
    """Procesează un grup: rulează IG + faithfulness pe fiecare articol."""
    ...
    for i, (_, row) in enumerate(df_grup.iterrows(), 1):
        ...
        # Salvăm HTML pentru primele 5 exemple din fiecare grup
        html_file = None
        if i <= 5:
...
def agrega_rezultate_ig(toate_rezultatele, lime_results=None):
    """
    Agregă rezultate IG per grup și (opțional) compară cu rezultate LIME.

    Dacă lime_results e furnizat, calculăm head-to-head:
    ...
    # Comparație head-to-head LIME vs IG (dacă avem rezultate LIME)
    comparatie_lime_ig = {}
    if lime_results is not None:
        ...
        # Adăugăm coloana faith_auc_lime pe rândurile noastre
...
def genereaza_markdown(agregari, teste_intergrupuri, comparatie_lime_ig,
    ...
    """Generează raport markdown comparativ LIME vs IG."""
    md = [
    ...
    # Comparație IG vs LIME pe Grup B (cls1 baseline)
...
def main():
    ...
    # Încărcare rezultate LIME (opțional)
    lime_results = None
