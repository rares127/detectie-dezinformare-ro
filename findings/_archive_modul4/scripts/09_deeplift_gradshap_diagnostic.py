...
Scop: După eșecul IG (completeness ~0.5, vezi findings_lime_vs_ig_l3.md), încercăm
două metode alternative gradient-based pe ACELEAȘI 100 articole (4 grupuri × 25,
seed 42) ca în L1a și L3, pentru triangulare metodologică completă.

  - DeepLift (Shrikumar et al. 2017): rescaled gradients, NU integrare pe path
    → mai rezistent la saturarea XLM-R fine-tuned decât IG
  - LayerGradientShap (Lundberg & Lee 2017): aproximare SHAP prin sampling
    multiple baselines → estimare stocastică, robustă la non-liniaritate locală

Predicție realistă (din handoff):
  - 35% una din ele converge bine (completeness < 0.1)
  - 65% ambele eșuează similar IG → finding metodologic: 4 metode XAI testate,
    toate cu limitări specifice → triangulare solidă pentru capitolul Limitări

Indiferent de rezultat, valoarea științifică:
  - Demonstrăm sistematic că saturarea modelului e un factor structural, NU
    o limitare a unei singure metode XAI
  - Confirmăm independent stylistic fingerprint (faith_auc cls0 vs cls1)
  - Justificăm de ce modul 3 (similaritate semantică) rămâne explicabilitatea
    principală a sistemului — e nativă, robustă, cross-source

Metrici per articol (consistent cu L1a și L3):
  - top_features: top-15 cuvinte după |atribuție|
  - faith_auc: drop predicție când ștergem top-K cuvinte (k=1,3,5,10)
  - completeness_relative: |sum(attr) - (logit_input - logit_baseline)| / |target|
    (DeepLift are axiomă completeness; GradShap NU are dar raportăm aproximare)
...
Output:
  ...
  - deeplift_html_l4/ — 5 vizualizări HTML per grup (20 total)
  - gradshap_html_l4/ — 5 vizualizări HTML per grup (20 total)
...
Usage:
    ...
    # Rulare finală (n=25)
    # ... aceleași argumente, dar --n_per_group 25
"""
...
# Top-K pentru extragerea cuvintelor importante (consistent cu L1a și L3)
TOP_K_WORDS = 15

# K-uri pentru faithfulness deletion (consistent cu L1a și L3)
K_VALUES_DELETION = (1, 3, 5, 10)

# Configurație DeepLift
DEEPLIFT_MULTIPLY_BY_INPUTS = True  # default Captum, formulă comparabilă cu IG

# Configurație GradientShap
GRADSHAP_N_SAMPLES = 20   # număr de eșantioane stocastice (handoff §3.2)
GRADSHAP_STDEVS = 0.0     # zero = baselines pure, fără zgomot Gaussian
...
class ModelWrapperLogits(torch.nn.Module):
    ...
    Motivație:
      - LayerDeepLift cere `model: nn.Module` ca prim argument (NU forward_func).
        Pentru Captum, semnătura forward trebuie să accepte tensorele ca argumente
        poziționale în ordinea exactă (Captum apelează `model(*inputs, *args)`).
      - LayerGradientShap acceptă forward_func, dar implementarea internă
        funcționează mai stabil pe transformere când forward returnează direct
        un tensor (nu un obiect ModelOutput cu .logits).

    Soluția: wrapper-modul care expune un forward(input_ids, attention_mask)
    pozițional, returnând direct tensorul de logits.

    Acest wrapper rezolvă Issue #771 și #678 din captum repo (probleme cu
    ...
    def __init__(self, model):
        ...
        # ușor în LayerDeepLift / LayerGradientShap fără a parcurge .model.roberta...
        self.embeddings = model.roberta.embeddings.word_embeddings

    def forward(self, input_ids, attention_mask):
        """Returnează logits ca tensor (N, num_classes)."""
        return self.model(input_ids=input_ids, attention_mask=attention_mask).logits


class ModelWrapperEmbeddings(torch.nn.Module):
    """
    Wrapper specializat pentru GradientShap — primește embeddings (float), NU input_ids (long).

    Motivație (Issue #771 Captum):
      LayerGradientShap aplică NoiseTunnel intern, care adaugă zgomot Gaussian
      peste `inputs`. Dacă inputs sunt input_ids (torch.long, indici discreți pentru
      embedding lookup), zgomotul îi convertește în float, iar embedding-ul cere Long/Int
      → RuntimeError: 'Expected tensor for argument indices to have Long, but got Float'.

    Soluția standard (din tutorialele Captum BERT/RoBERTa):
      1. Calculăm noi embeddings ÎN AFARA wrapperului → tensor float
      2. Wrapperul primește embeddings float și îi pasează modelului prin `inputs_embeds=`
         (API HuggingFace standard, suportat nativ de XLM-RoBERTa)
      3. NoiseTunnel poate acum adăuga zgomot Gaussian pe float-uri fără probleme
      4. GradientShap atribuie direct pe spațiul embedding (mai natural pentru analiză)
    """
    ...
    def calculeaza_embeddings(self, input_ids):
        """Helper: convertește input_ids → embeddings float pentru GradShap."""
        return self.embedding_layer(input_ids)


def construieste_predict_proba(model, tokenizer, device, max_length=256, batch_size=16):
    """
    Construiește funcția predict_proba pentru faithfulness deletion.
    IDENTICĂ cu cea din L1a și L3 — pentru comparație directă a metricii deletion.
    """
    def predict_proba(texts):
        """Returnează matrice (N, 2) de probabilități softmax."""
        all_probs = []
        with torch.no_grad():
...
def construieste_baseline_ids(input_ids, tokenizer, mode="pad"):
    """
    Construiește un tensor de IDs pentru baseline, păstrând tokenii speciali
    CLS și SEP la pozițiile lor (consistent cu metodologia IG din L3).

    mode:
      - "pad": înlocuiește non-speciali cu PAD (recomandare standard)
      - "shuffled": permută aleator non-speciali (pentru diversitate GradShap)

    Returnează tensor de aceeași shape ca input_ids.
    """
    ...
    if mode == "pad":
        # PAD pe toate pozițiile non-speciale
        ...
        # Permutăm aleator tokenii non-speciali (păstrăm CLS/SEP pe loc)
        non_speciali_idx = []
        non_speciali_vals = []
        for i in range(input_ids.shape[1]):
            tok = input_ids[0, i].item()
        ...
        # Permutăm valorile (cu numpy pentru reproducibilitate prin seed global)
...
def agrega_tokens_la_cuvinte(text, offsets, attr_per_token, input_ids, tokenizer):
    """
    Agregă atribuțiile per token la nivel de cuvânt folosind offset_mapping.

    IDENTIC cu funcția din 08_ig_l3_diagnostic.py — pentru consistență totală
    între metodele XAI gradient-based (vocabular comparabil în top-K).

    XLM-R folosește SentencePiece BPE — un cuvânt poate fi împărțit în mai mulți
    tokeni. Sumăm atribuțiile tokenilor aceluiași cuvânt (delimitat de spațiu).

    Returns:
        list de (cuvânt, atribuție_agregată) în ordinea apariției în text
    """
    special_ids = set()
    ...
    for i, (start, end) in enumerate(offsets):
        # Sărim tokenii speciali (CLS, SEP, PAD) — au offset (0, 0)
        if input_ids[i] in special_ids or start == end == 0:
            continue

        # Verificăm dacă tokenul curent e începutul unui cuvânt nou
...
def calculeaza_faithfulness_deletion(text, top_features, predict_proba, label_pred,
    ...
    """
    Faithfulness deletion AUC — IDENTIC cu L1a și L3.

    Șterge top-k cuvinte (după |atribuție|) și măsoară drop predicție.
    AUC mai mare = metoda XAI identifică cuvinte cu impact cauzal real.
    """
...
def calculeaza_deeplift_pe_articol(text, label_pred, model, tokenizer, device,
    ...
    """
    Calculează atribuții DeepLift (Shrikumar et al. 2017) pentru un articol.

    Diferență față de IG:
      - Nu integrează pe path → un singur backward pass cu rescaled gradients
      - Mai rezistent la saturare: folosește diferența f(x) - f(baseline) direct
        împărțită la (x - baseline), evitând zona de gradient ~0 a sigmoid/softmax
      - Are axiomă completeness similară cu IG (suma atribuțiilor ≈ Δ logit)

    Implementare Captum:
      - LayerDeepLift cu embedding layer → atribuții per token
    ...
    # Baseline = PAD ids cu CLS/SEP păstrate
    ...
    # LayerDeepLift cere un nn.Module ca prim argument și apelează forward
    # pozițional cu (inputs, *additional_forward_args).
    ...
    # Calculăm atribuțiile cu return_convergence_delta pentru sanity check
    ...
    # Reducem dimensiunea hidden cu sumă (păstrează semnul, comparabil cu IG)
    attr_per_token = attributions.sum(dim=-1).squeeze(0).detach().cpu().numpy()

    # Sanity check completeness: suma atribuțiilor ≈ logit_input - logit_baseline
    with torch.no_grad():
    ...
    # Probabilități pe input și baseline (pentru raportare)
    with torch.no_grad():
...
def calculeaza_gradshap_pe_articol(text, label_pred, model, tokenizer, device,
                                       ...
                                       seed=42):
    """
    Calculează atribuții GradientShap (Lundberg & Lee 2017) pe spațiul embedding.

    Mecanică:
      - Eșantionează aleator între input și mai multe baselines
      - Calculează gradient pe traseul random (un fel de IG stocastic)
      - Aproximare SHAP: media gradienților ponderați
      - Avantaj față de IG: aproximare stocastică, mai robustă la non-liniaritate

    Workaround issue #771 Captum (embedding layers + NoiseTunnel):
      Inițial am folosit LayerGradientShap pe word_embeddings, dar NoiseTunnel
      adăuga zgomot Gaussian peste input_ids (Long), ceea ce le converteste la Float
      → embedding lookup eșua cu „Expected Long, got Float".
      SOLUȚIA: calculăm noi explicit embeddings (float) și folosim GradientShap
      direct pe spațiul embedding, prin wrapper cu inputs_embeds= (API HuggingFace).
      Acum NoiseTunnel adaugă zgomot pe float-uri, fără conflict de tip.

    Setup baseline-uri (3 tipuri pentru diversitate stocastică):
      - PAD embeddings cu CLS/SEP păstrate (referință standard)
      - 2× Shuffled non-speciali embeddings (referință semantică nulă)
      Pool-ul are 3 elemente, GradShap eșantionează între ele cu n_samples=20.

    NOTĂ importantă: GradientShap NU are axiomă strictă de completeness.
    Raportăm `convergence_delta` mediu peste sample-uri ca proxy de stabilitate.

    Returns: dict similar cu DeepLift (cuvinte_atributii, top_features, etc.)
    """
    # Setăm seed local pentru reproducibilitate sampling GradShap
    ...
    # Construim 3 baseline-uri ID-uri (rămân Long pentru lookup ulterior)
    ...
    # primește embeddings (float), nu input_ids (long) → NoiseTunnel poate
    # adăuga zgomot Gaussian fără să rupă embedding lookup
    wrapper = ModelWrapperEmbeddings(model)
    wrapper.eval()

    # ---- WORKAROUND issue #771 ----
    # Calculăm explicit embeddings pentru input și baselines (output: float)
    # → Captum va atribui pe spațiul embedding (float), evitând incompatibilitatea Long/Float
    with torch.no_grad():
    ...
    # GradientShap (varianta NON-Layer) — atribuie direct pe spațiul embedding
    # care e deja float, deci nu mai apare conflict Long/Float în NoiseTunnel
    ...
    # Verificare shape compatibility (lungime și hidden_dim trebuie să se potrivească)
    ...
    # Verificare dtype: ambele trebuie să fie float (NU long) acum
    if not input_embeds.dtype.is_floating_point:
        raise RuntimeError(f"input_embeds nu e float! {diagnostic}")

    # Apel Captum cu inputs și baselines pe spațiul embedding (float)
    ...
    # Reducem dimensiunea hidden cu sumă (atribuțiile sunt pe spațiul embedding)
    attr_per_token = attributions.sum(dim=-1).squeeze(0).detach().cpu().numpy()

    # Pentru GradShap, "completeness" e aproximativ — folosim baseline_pad_ids ca referință
    # (cel mai aproape de IG/DeepLift pentru comparație head-to-head)
    with torch.no_grad():
        logit_input = model(input_ids=input_ids, attention_mask=attention_mask).logits[0, label_pred].item()
    ...
    # Probabilități pentru raportare
    with torch.no_grad():
        prob_input = float(torch.softmax(
            model(input_ids=input_ids, attention_mask=attention_mask).logits, dim=-1
        )[0, label_pred].item())
    ...
    # delta de la GradShap e tensor (n_samples,) — luăm mean abs ca proxy stabilitate
...
# ============================================================================
# SELECȚIE GRUPURI (IDENTIC L1a și L3)
# ============================================================================

def selecteaza_grupuri(baseline_preds, baseline_test, loso_preds, loso_test,
                        n_per_group, seed):
    """
    IDENTIC cu L1a și L3 — selectează aceleași 4 grupuri × n_per_group articole.
    Folosim același seed → exact aceleași articole → comparație head-to-head 4-way.
    """
...
def salveaza_html_atributii(text, cuvinte_atributii, label_pred, label_true,
    ...
    """
    Salvează HTML cu atribuții colorate (verde=pozitiv, roșu=negativ).
    Generic pentru DeepLift și GradientShap (parametrul `titlu_metoda`).
    """
...
# ============================================================================
# PROCESARE GRUP — RULĂM AMBELE METODE PE FIECARE ARTICOL
# ============================================================================

def proceseaza_grup(nume_grup, df_grup, model, tokenizer, device,
                      html_dir_dl, html_dir_gs, max_length, seed):
    """
    Procesează un grup: pentru fiecare articol rulăm DEEPLIFT și GRADSHAP
    pe același input → metrici head-to-head pe același articol.
    """
    ...
    for i, (_, row) in enumerate(df_grup.iterrows(), 1):
        ...
        # Smoke test detectase eșec silențios pe TOATE articolele. Aici capturăm
        # tipul excepției, mesajul, ultimele frame-uri de traceback și informații
        ...
        try:
            ...
            # Afișăm în consolă: tipul, mesajul, ultimele 4 linii de traceback
            # (Captum-ul intern unde a crăpat — cel mai informativ pentru debug)
            ...
            # Salvăm tracebackul complet în fișier dedicat pentru analiză post-mortem
        ...
        print(f"  [{i:2d}/{len(df_grup)}] {row['id']} | {str(sursa)[:12]:12s} | "
              ...
              f"{gs_str} | DL={t_dl:.1f}s GS={t_gs:.1f}s")
...
def agrega_rezultate(toate_rezultatele, lime_results=None, ig_results=None):
    """
    Agregă rezultate per grup pentru DeepLift și GradShap, plus comparație
    head-to-head 4-way cu LIME (din findings_lime_l1a.json) și IG
    ...
    for grup in ["A", "B", "C", "D"]:
        ...
        # GradShap (poate avea None-uri dacă a eșuat)
    ...
    # Mann-Whitney intergrupuri pe faith_auc DL și GS
    teste_intergrupuri = {}
    ...
    for metric in ["dl_faith_auc", "gs_faith_auc"]:
        for g1, g2 in perechi:
            ...
            try:
                ...
                # Calculăm și mean și median pentru fiecare grup, plus diff pe ambele
                # — necesar pentru raportare onestă pe distribuții skewed (mean ≠ median)
    ...
    # ---- Comparație 4-way pe articol (LIME + IG + DL + GS) ----
    comparatie_4way = {}

    # Map id → faith LIME și faith IG (din JSON-uri externe)
    lime_per_id = {}
    if lime_results:
    ...
    for grup in ["A", "B", "C", "D"]:
        for col_a, col_b, key in perechi_metode:
            ...
            if len(sub) < 3:  # Wilcoxon necesită cel puțin câteva diferențe non-zero
                continue
            try:
                # Verificăm că nu sunt toate diferențele zero
    ...
    # ---- Jaccard top-5 cuvinte între metode ----
    jaccard_4way = {}

    # Construim mapping id → top5 pentru fiecare metodă
    lime_top5_per_id = {}
    if lime_results:
        for r in lime_results.get("rezultate_per_articol", []):
...
def genereaza_markdown(agregari, teste_intergrupuri, comparatie_4way,
    ...
    """Generează raport markdown cu toate cifrele 4-way (LIME + IG + DL + GS).

    Args:
        toate_rezultatele: lista raw de rezultate per articol (opțional).
            Necesar pentru secțiunea 7bis ('Finding D<C') care extrage articolele
            individuale cu drop NEGATIV major (modelul folosește narațiune distribuită,
    ...
    # ---- Tabelul HEADLINE: comparație 4-way ----
    md.extend([
        "",
    ...
    # ---- Secțiunea 7bis: Finding D<C (narațiune distribuită) ----
    # Generată DOAR dacă avem toate_rezultatele și avem grup D + C cu date
    if toate_rezultatele is not None:
        ...
        if len(sub_c) > 0 and len(sub_d) > 0:
            ...
            # Sortăm D crescător după dl_faith_auc (cele mai negative primele)
    ...
    # Comparație DL vs LIME pe Grup B (cls1 baseline) — testul cheie pentru fingerprint
...
def main():
    ...
    # ---- Încărcare rezultate LIME și IG (pentru comparație 4-way) ----
    lime_results = None
    ...
    # ---- Procesare A și B (model baseline) ----
