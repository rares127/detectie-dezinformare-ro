...
Output:
  - findings/benchmark_v4_post_curatare.md + .json (rezultate + interpretare)
...
RAPORT_MD = Path("findings/benchmark_v4_post_curatare.md")
RAPORT_JSON = Path("findings/benchmark_v4_post_curatare.json")
...
def _scrie_raport_md(raport: dict, path: Path) -> None:
    ...
    linii: list[str] = []
    linii.append("# Benchmark v4 — POST-CURĂȚARE test set extern")
    linii.append("")
    linii.append(
        "Re-rulare benchmark v4 pe `subset_benchmark_v3_curat.parquet` "
        "(test set curățat de cookie banners HotNews/Pro TV/Libertatea + "
        "comentarii). Vezi `curatare_test_extern_cookies.md` pentru detalii "
        "curățare. Acest raport e comparabil direct cu `benchmark_v4.md` "
        "pentru a cuantifica artefactul de cookie banners."
    )
    linii.append("")
