...
Schimbari fata de v1:
    - Citeste subset_benchmark_v2.parquet (cls0 din surse externe,
...
Output:
    - findings/benchmark_embeddings_v2.md
    - findings/benchmark_embeddings_v2.json
...
CALE_OUT_MD = Path("findings/benchmark_embeddings_v2.md")
CALE_OUT_JSON = Path("findings/benchmark_embeddings_v2.json")
...
def genereaza_raport_md(rezultate: dict) -> str:
    ...
    linii = [
        ...
        "",
        "**Reparație față de v1:** articolele cls0 vin acum din surse EXTERNE "
        ...
        f"**Subset v2:** {meta['n_articole']} articole "
...
def main() -> None:
    ...
    print("=" * 70)
    print("BENCHMARK EMBEDDINGS v2 — MODULUL 3")
    print("=" * 70)
    ...
    if not CALE_SUBSET.exists():
        raise FileNotFoundError(
            ...
            f"Rulează mai întâi selecteaza_subset_benchmark_v2.py."
        )

    df_corp = pd.read_parquet(CALE_CORPUS)
    df_sub = pd.read_parquet(CALE_SUBSET)
    print(f"\nCorpus: {len(df_corp)} propoziții")
    print(f"Subset v2: {df_sub['articol_id'].nunique()} articole, "
