...
Scop: testam ipoteza ca R² LIME scazut pe cls1 e cauzat de:
  (i)  saturare softmax (artefact tehnic, fixabil cu logits)
  (ii) stylistic fingerprint (limitare intrinseca a modelului)

Comparam 4 grupuri, fiecare cu n=25 articole:
  ...
  - Grup C: FN LOSO-V pe Veridica (modelul LOSO-V rateaza propaganda)
  - Grup D: TP LOSO-V pe Veridica (modelul LOSO-V prinde propaganda fara amprenta)

Trei metrici per articol:
  (a) R² pe softmax probabilities (ca in scriptul vechi 06_lime_xlmr_v2.py)
  (b) R² pe logits raw (NEW — testeaza ipoteza saturare)
  (c) Faithfulness deletion AUC (NEW — cata parte din predictie explica top-k cuvinte)

Ipoteze testate:
  H1: R²(logits) > R²(proba) pe ambele clase (efect saturare prezent global)
  H2: Δ R² intre cls0 si cls1 persista si pe logits (stylistic fingerprint real)
  H3: Faithfulness deletion mai buna pe cls0 vs cls1 (top-k LIME au impact cauzal mai mare)

Output:
  - findings_lime_l1a.json — date raw per articol (pentru re-analiza)
  - findings_lime_l1a.md — raport cu tabele agregate, IC 95%, Mann-Whitney
  - lime_html_l1a/ — vizualizari HTML pe predict_proba (selectie reprezentativa)
...
# Configuratie LIME — identica cu scriptul vechi pentru comparabilitate
NUM_FEATURES = 15
NUM_SAMPLES = 1000
BOW = False  # pastreaza ordinea token-urilor (important pentru transformer)
...
def construieste_predict_fns(model, tokenizer, device, max_length=256, batch_size=16):
    """
    Returneaza doua functii predict pentru LIME:
      - predict_proba: returneaza softmax probabilities (matrice N×2)
      - predict_logits: returneaza raw logits (matrice N×2)

    Logits-ul evita saturarea softmax pe modele high-confidence:
      - softmax cls1 ~0.9996 → perturbari produc Δ ~10⁻⁴ (zgomot)
      - logits cls1 ~+8 → aceleasi perturbari produc Δ ~1-3 (semnal)
    """

    def predict_proba(texts):
        """Predictie pe softmax — interfata standard LIME."""
        all_probs = []
        with torch.no_grad():
    ...
    def predict_logits(texts):
        """
        Predictie pe logits raw — LIME va antrena regresie liniara pe logits.

        IMPORTANT: LIME se asteapta la matrice (N, num_classes) de „scoruri".
        Logits-urile satisfac asta. R² rezultat are aceeasi semnificatie
        (cat de bine aproximeaza regresia liniara comportamentul modelului),
        dar pe scala non-saturata.
        """
        all_logits = []
        with torch.no_grad():
            for i in range(0, len(texts), batch_size):
...
def calculeaza_faithfulness_deletion(text, top_features, predict_proba, label_pred,
    ...
    """
    Faithfulness deletion: sterge top-k cele mai importante cuvinte (dupa LIME)
    si masoara cat scade probabilitatea predictiei initiale.

    Returneaza AUC-ul curbei (k → drop predictie). AUC mai mare = LIME identifica
    cuvinte cu impact cauzal real asupra modelului.

    Args:
        text: textul original (string)
        top_features: list de (cuvant, weight) sortat descrescator dupa |weight|
        predict_proba: functia de predictie (returneaza matrice N×2)
        label_pred: indexul clasei prezise (0 sau 1)
        k_values: ce valori de k sa testam pentru deletion
    ...
    # Sortam features dupa |weight| descrescator (cele mai importante prima)
    ...
    for k in k_values:
        ...
        # Construim text cu top-k cuvinte sterse (case-insensitive)
        cuvinte_de_sters = set(w.lower() for w, _ in features_sorted[:k])
        # Tokenizare simpla pe whitespace (consistency cu LIME bow=False)
        tokens = text.split()
...
def selecteaza_grupuri(baseline_preds, baseline_test, loso_preds, loso_test,
                        n_per_group, seed):
    """
    Selecteaza cele 4 grupuri de articole stratificat.

    Returneaza un dict cu 4 chei (A, B, C, D), fiecare cu DataFrame de
    n_per_group articole imbogatite cu coloana text.
    """
    ...
    # Grup A: TP cls0 baseline (Digi24 + G4Media), stratificat pe sursa
    ...
    # Grup B: TP cls1 baseline (Veridica + Stopfals), stratificat pe sursa
    ...
    # Eliminam articolele fara text disponibil inainte de esantionare
...
def ruleaza_lime_pe_articol(text, label_pred, predict_proba, predict_logits,
    ...
    """
    Ruleaza LIME pe un articol cu ambele functii predict (proba + logits).
    Returneaza dict cu R² si top features pentru ambele.
    """
    rezultat = {
...
def proceseaza_grup(nume_grup, df_grup, model, tokenizer, device, html_dir,
    ...
    """
    Proceseaza un grup: ruleaza LIME pe fiecare articol, calculeaza metrici,
    salveaza HTML-uri pentru cateva exemple reprezentative.
    """
    ...
    for i, (_, row) in enumerate(df_grup.iterrows(), 1):
        ...
        # Salvam HTML pentru primele 5 exemple din fiecare grup (reprezentativ)
        html_file = None
        if i <= 5:
...
def agrega_rezultate(toate_rezultatele):
    """Calculeaza statistici agregate per grup si teste statistice intre grupuri."""
    ...
    # Mann-Whitney U intre grupuri (test non-parametric)
    teste = {}
    ...
    # Wilcoxon pereche pe articol: logits vs proba (in cadrul aceluiasi grup)
    teste_pereche = {}
    for grup in ["A", "B", "C", "D"]:
        sub = df[df["grup"] == grup]
        if len(sub) == 0:
...
def genereaza_markdown(agregari, teste, teste_pereche, n_per_group, seed):
    """Genereaza raportul markdown cu interpretare automata."""
    md = [
    ...
    # H2: stylistic fingerprint persista pe logits
    ...
    # H3: faithfulness diferentiala A vs B
    if "A" in agregari and "B" in agregari:
...
def main():
    ...
    # Concatenam si deduplicam pe id (ultima valoare castiga pentru duplicate)
    loso_test_dfs = []
    ...
    # Verificare coloana text (baseline; LOSO deja verificat la incarcare)
